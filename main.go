...
func (c *DNSMadeEasyProviderSolver) getDnsClient(ch *webhookapi.ChallengeRequest, cfg DNSMadeEasyProviderConfig) (*GoDNSMadeEasy.GoDMEConfig, error) {
  ...
  if apiKey == "" {
    ref := cfg.APIKeyRef
  	...
  	secret, err := c.client.CoreV1().Secrets(ch.ResourceNamespace).Get(context.TODO(), cfg.APIKeyRef.LocalObjectReference.Name, metav1.GetOptions{})
  	if err != nil {
  		return nil, err
  	}
  ...
  if apiSecret == "" {
    ...
    secret, err := c.client.CoreV1().Secrets(ch.ResourceNamespace).Get(context.TODO(), cfg.APISecretRef.LocalObjectReference.Name, metav1.GetOptions{})
  	if err != nil {
  		return nil, err
  	}
